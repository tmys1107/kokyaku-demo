...
## 議事録・引き継ぎ運用（重要）
- このプロジェクトは将来、妻が別アカウントの Claude Code で引き継ぐ可能性がある。
- 引き継ぎ用の議事録は **`docs/議事録.md`** に残す。リポジトリ内に置くことでアカウントをまたいで共有できる（※Claudeの自動メモリ機能はユーザー個人のものなので引き継げない）。
- **運用ルール**：作業の区切りごとに `docs/議事録.md` を更新する。決定事項・実装内容・次アクションを簡潔に記し、**不要になった情報は削除して常に最新の引き継ぎ状態を保つ**（累積ログにしない）。
- 区切りでコミット・プッシュし、GitHub に最新の議事録が残るようにする。
